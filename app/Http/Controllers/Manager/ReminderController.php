...
class ReminderController extends Controller
{
    ...
    public function index(){
        ...
        $reminders = Reminder::where('recipient_type', 'manager')->where('recipient_id', $this->user->id)->orderBy('next_reminder_date', 'desc');
    ...
    public function show($uuid){
        $reminder = Reminder::where('uuid', $uuid)->where('recipient_type', 'manager')->where('recipient_id', $this->user->id)->first();
        if(empty($reminder)){
            return response([
                'status' => 'failed',
                'message' => 'No Reminder was fetched' 
    ...
    public function send_reminder(StoreReminderRequest $request, $uuid){
        ...
        NotificationController::store('tenant', $tenant->user_id, "Reminder", "Your Manager, {$this->user->name} just set a reminder for you", 'reminders', $reminder->uuid);
